...
export default function CustomerWorkDetails() {
  ...
  return (
    <PortalPage title="Work Details" subtitle="All farm work orders and their status" navItems={CUSTOMER_NAV}>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {workOrders.map((w) => <WorkOrderCard key={w.id} workOrder={w} />)}
        {workOrders.length === 0 && <p className="col-span-3 text-center text-[#8fac9a] py-10">No work orders yet</p>}
      </div>
    </PortalPage>
  );
}
